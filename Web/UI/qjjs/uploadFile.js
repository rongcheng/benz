function table(obj){
	...
	if(!this.$(obj.id)) return alert("ID对象找不到哦");
	var
	my=this,
...
table.prototype.allFiles=function(i,string){
	...
	this.allFile_1=function(str){//检查是否重复
		if(str){
			var strs=[],
			len=str.length;
			for(var i=0;i<len;i++){
	...
	this.allFile_3=function(str){//单个上传完毕
		this.cc(this.$("allFiles_p1"),{"innerHTML":"已上传完毕 [<strong>"+str[0]+"</strong>]，正在处理..."});
	...
	var
	text1=["正在检查是否重复...","发现有重名!","文件名与括号内已上传文件的原文件名相同，请检查是否为相同文件，如果不相同请更名后再上传，如相同则请取消上传此文件。"],
	text4=["全部文件上传成功！ <span style='font-size:12px;font-weight:normal;'>现在开始：<a href='/Upload/Edit.aspx' style='color:#380!important;'>编辑信息>></a></span>","上传完毕！以下文件未完成："],
	text6=["停止上传"];
