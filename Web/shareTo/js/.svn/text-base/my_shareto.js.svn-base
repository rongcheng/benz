	(function(){
		var d = document,
			...
			ie = !!d.all,ua = navigator.userAgent.toLowerCase(),
			...
			div1 = creElm({style:"position:absolute;z-index:100000;display:none;top:50%;left:50%;"}),
			timer,inputTimer,list,clist,as,texts={},
		...
		sharetoBtnContain = {
			...
			disappear : function(b) {
				var c = window.event || b, t = c.srcElement || c.target, contain = div1.contains ? div1
						.contains(t)
						: !!(div1.compareDocumentPosition(t) & 16);
				if (!contain) {
			...
			center : function(){
				if(!this.script){
					this.script = creElm({src:'http://shareto.googlecode.com/svn/trunk/js/shareto_button_window.js',charset:'utf-8'},'script',head);
		...
		ie?d.attachEvent("onclick",sharetoBtnContain.disappear):d.addEventListener("click",sharetoBtnContain.disappear,false);
...
//							+ (screen.height - 500) / 2);
			...
			window
					.open(
							'http://apps.hi.baidu.com/share/?url=' + encodeURIComponent(location.href),
							'baiduhi',
